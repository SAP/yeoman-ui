...
export default {
  ...
  methods: {
    ...
    onAnswered(answers, issues) {
      ...
      if (currentPrompt) {
        ...
        if (currentPrompt.answers.generator) {
          this.isFioriProject = _.includes(currentPrompt.answers.generator, "fiori") ? true : false;
        }
    ...
    async showPrompt(questions, name) {
      ...
      if (this.nextButtonText === "Finish" && this.isFioriProject) {
        const message = "Generated project will not open in a new workspace";
