...
function getTextForAnswer(question, answer) {
  if (Array.isArray(question._choices)) { // `_choices` is the normalised version of `choices` (Array | Function) used by the Form.vue so we re-use rather than re-processing
    let answers = [answer];
